...
def format_and_save_best_locations_to_excel(best_locations, output_path):
    ...
    # Зберігаємо в Excel
    best_locations_df.to_excel(
        output_path + "new_store_best_locations.xlsx", index=False
    )
...
def calculate_total_location_score(
    potential_location_coord, pops_coords, other_shops_effects_on_each_pop, alpha
):
    # Знаходимо відстань від локації до всіх популяцій
    lat_diff = (pops_coords[:, 0] - potential_location_coord[0]) * 111
    lon_diff = (
        (pops_coords[:, 1] - potential_location_coord[1])
        * 111
        * np.cos(np.radians(pops_coords[:, 0]))
    )
    ...
    pops_rel_weight_by_dist_factor = calculate_pop_rel_weight_by_dist(
        distances_to_store, alpha
    )
    # Від вагів популяцій з накоаденим штрафом за відстань додатково віднімаємо порахований раніше вплив інших магазинів
    pops_rel_weight_by_dist_and_other_shops_factor = pops_rel_weight_by_dist_factor * (
        1 - other_shops_effects_on_each_pop
    )
...
def calculate_other_shops_effect(
    distances_matrix, silpo_and_other_corps_shops, alpha=1
):
    # Обчислюємо базовий вплив для всіх магазинів (за допомогою альфи можна корегувати штраф за відстань)
    other_shops_impact_matrix = calculate_pop_rel_weight_by_dist(
        distances_matrix, alpha
    )
...
def calculate_distances_matrix(pops_coords, silpo_and_other_shops_coords):
    ...
    if len(silpo_and_other_shops_coords) == 0:
        return np.zeros(
            (len(pops_coords), 0)
        )  # Матриця відстаней з нульовою кількістю колонок

    # Створюємо два масиви (широта і довгота) в одному циклі
    shops_lats, shops_lons = np.array(
        [(coord[1], coord[2]) for coord in silpo_and_other_shops_coords], dtype=float
    ).T  # Транспонування для розділення на широти і довготи
    ...
    lon_diff = (
        (pops_coords_np_arr[:, 1, np.newaxis] - shops_lons)
        * 111
        * np.cos(np.radians(pops_coords_np_arr[:, 0, np.newaxis]))
    )
...
def get_locations_with_best_score(
    pops_coords, silpo_and_other_shops_coords, alpha=1, step_size=0.001, max_workers=8
):
    ...
    distances_from_pops_to_shops_matrix = calculate_distances_matrix(
        pops_coords, silpo_and_other_shops_coords
    )
    # Розраховуємо ефект (штраф) який створюють інші магазини на популяції в залежності від дистанції між ними
    other_shops_effects_on_each_pop = calculate_other_shops_effect(
        distances_from_pops_to_shops_matrix, silpo_and_other_shops_coords, alpha
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for y in np.arange(y_max, y_min, -step_size):
            for x in np.arange(x_min, x_max, step_size):
                ...
                future = executor.submit(
                    calculate_total_location_score,
                    potential_location_coord,
                    pops_coords,
                    other_shops_effects_on_each_pop,
                    alpha,
                )
        ...
        for future in as_completed(future_to_location):
            try:
                ...
                all_potential_locations_score.append(
                    [potential_location_score, potential_location_coord]
                )
            except Exception as e:
                print(f"Error in task: {e}")

    all_potential_locations_score_sorted = sorted(
        all_potential_locations_score, key=lambda x: x[0], reverse=True
    )
...
def get_prepared_coords(competitors_shops_prepared):
    ...
    silpo_shops_coords_with_corp_name = [
        [
            "Silpo",
            lat,
            lon,
        ]
        for lat, lon in silpo_shops_coords
    ]

    silpo_and_other_shops_coords = (
        silpo_shops_coords_with_corp_name + competitors_shops_prepared
    )
...
def find_best_locations(competitors_shops):
    print(
        'Відбувається "расклад" Таро для пошуку найкращої локації, це може зайняти декілька хвилин...'
    )
    ...
    best_locations = get_locations_with_best_score(
        pops_coords,
        silpo_and_other_shops_coords,
        alpha=1,
        step_size=0.001,
