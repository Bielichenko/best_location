...
def assign_populations_to_shop():
    ...
    populations_data_assigned_with_shops["closest_shop"] = (
        indices.flatten()
    )  # Індекси найближчих магазинів
...
def calculate_shops_metrics(populations_data_assigned_with_shops):
    ...
    silpo_shops_data_with_metrics["avg_distance_to_pops"] = grouped_populations[
        "distance_to_shop"
    ].mean()
    silpo_shops_data_with_metrics["sum_pops_metric"] = grouped_populations[
        "metric population"
    ].sum()
    silpo_shops_data_with_metrics["avg_pops_metric"] = grouped_populations[
        "metric population"
    ].mean()
...
def calc_correlations_with_binding_approach():
    ...
    silpo_shops_data_with_metrics = calculate_shops_metrics(
        populations_data_assigned_with_shops
    )
    ...
    for col in ["avg_distance_to_pops", "sum_pops_metric", "avg_pops_metric"]:
        valid_rows = (
            silpo_shops_data_with_metrics[["Metric Store", col]].dropna().astype(float)
        )

        if len(valid_rows) > 1:
            corr_results[col] = calculate_corr_and_pvalue(
                valid_rows["Metric Store"], valid_rows[col]
            )
        else:
            print(
                f"Недостатньо даних для обчислення кореляції між {col} та Metric Store"
            )

    # Збереження результатів
    silpo_shops_data_with_metrics.to_excel(
        f"{output_dir}/corr_binding_approach_shops_data.xlsx", index=False
    )
    corr_results.to_excel(
        f"{output_dir}/corr_binding_approach_results.xlsx", index=True
    )
