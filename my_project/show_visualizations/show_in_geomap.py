...
def show_in_geomap(
    is_need_to_show_store_and_pop_metric=False,
    best_location_coords=None,
    competitors_stores=[],
):
    # Перетворюємо populations_data на GeoDataFrame для використання методів GeoPandas
    populations_data["geometry"] = populations_data.apply(
    ...
    def calculate_population_sum_in_square(square, population_data):
        ...
        return (
            ...
            if is_need_to_show_store_and_pop_metric
            else population_metric_count
        )
    ...
    for square in grid_cells:
        ...
        grid_population.append({"geometry": square, "population": population_sum})
    ...
    folium_map = folium.Map(
        location=[(ymin + ymax) / 2, (xmin + xmax) / 2], zoom_start=13
    )
    ...
    for _, store in silpo_shops_data.iterrows():
        radius_if_need_to_show_store_metric = (
            (store["Metric Store"] - min_store_metric)
            / (max_store_metric - min_store_metric)
        ) * 10  # Діапазон від 10 до 30
        prepared_radius = (
            10
            if not is_need_to_show_store_and_pop_metric
            else radius_if_need_to_show_store_metric
        )
        folium.CircleMarker(
